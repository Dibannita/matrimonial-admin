...
@Component({
  selector: 'app-child-category-select',
  templateUrl: './child-category-select.component.html',
  styleUrls: ['./child-category-select.component.scss']
})
export class ChildCategorySelectComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy {

  // Dynamic Component Data
  @Input('controlName') controlName: FormControl;
  @Input('required') required: boolean = false;
  @Input('placeholder') placeholder: string = 'Select';
  @Input('data') data: ChildCategory;
  @Input('subCategoryId') subCategoryId: string;

  // Store Data
  filter = null;
  public dataList: ChildCategory[] = [];
  public filteredDataList: ReplaySubject<ChildCategory[]> = new ReplaySubject<ChildCategory[]>(1);
  searchDataList: ChildCategory[] = [];
  ...
  constructor(
    private childCategory:ChildCategoryService,
  ...
  ngOnChanges() {
    if (this.subCategoryId) {
      this.filter = { 'subCategory._id': this.subCategoryId }
      // Default Data
    ...
    this.searchCtrl.valueChanges
      .pipe(
        ...
        switchMap(search => {
          ...
          return this.childCategory.getAllChildCategories(filterData, search);
        })
      )
      .subscribe({
        next: res => {
          this.searching = false;
          this.searchDataList = this.subCategoryId ? [...this.searchDataList, ...res.data] : [];
          this.filteredDataList.next(this.subCategoryId ? res.data : []);
        },
        error: error => {
          this.searching = false;
          console.log(error)
  ...
  private getAllCategoryList() {
    ...
    this.childCategory.getAllChildCategories(filter, null)
      .subscribe({
        next: res => {
