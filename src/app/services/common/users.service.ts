...
export class UsersService {
  updateProductById(_id: string, data: any) {
  ...
  addUser(data: Users) {
    ...
    return this.httpClient.post<ResponsePayload>(API_USERS + 'signup', data);
  }

  insertManyUser(data: Users, option?: any) {
    const mData = { data, option };
    return this.httpClient.post<ResponsePayload>(
      API_USERS + 'insert-many',
      mData
    );
  }
  ...
  getAllUsers(filterData: FilterData, searchQuery?: string) {
    ...
    return this.httpClient.post<{
      ...
      count: number;
      success: boolean;
    }>(API_USERS + 'get-all', filterData, { params });
  }
  ...
  updateUsersById(id: string, data: Users) {
    return this.httpClient.put<{ message: string; success: boolean }>(
      API_USERS + 'update-data/' + id,
      data
    );
  }
  cloneSingleUser(id: string) {
    return this.httpClient.post<ResponsePayload>(API_USERS + 'clone', { id });
  }
  deleteMultipleUserById(ids: string[], checkUsage?: boolean) {
    ...
    return this.httpClient.post<ResponsePayload>(
      API_USERS + 'delete-multiple-data-by-id',
      { ids: ids },
      { params }
    );
  }
