...
@Component({
  selector: 'app-all-child-categories',
  templateUrl: './all-child-categories.component.html',
  styleUrls: ['./all-child-categories.component.scss']
})
export class AllChildCategoriesComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  // SEARCH AREA
  searchChildCategories: ChildCategory[] = [];
  searchQuery = null;
  ...
  ngAfterViewInit(): void {
    ...
    this.subForm = formValue.pipe(
      ...
      switchMap(data => {
        ...
        return this.childCategoryService.getAllChildCategories(filterData, this.searchQuery);
      })
    )
      .subscribe({
        next: (res => {
          this.searchChildCategories = res.data;
          this.childCategories = this.searchChildCategories;
   ...
   */
   private getAllCategories() {
    ...
    this.subDataSix = this.categoryService.getAllCategory(filterData, null)
      .subscribe(res => {
  ...
  private getAllChildCategories() {
    ...
    this.subDataOne = this.childCategoryService.getAllChildCategories(filterData, this.searchQuery)
      .subscribe({
        next: (res => {
  ...
  private deleteMultipleChildCategoryById() {
    this.spinner.show();
    this.subDataFour = this.childCategoryService.deleteMultipleChildCategoryById(this.selectedIds, true)
      .subscribe({
        next: (res => {
          this.spinner.hide();
          if (res.success) {
  ...
  private updateMultipleChildCategoryById(data: any) {
    this.spinner.show();
    this.subDataThree = this.childCategoryService.updateMultipleChildCategoryById(this.selectedIds, data)
      .subscribe({
        next: (res => {
          this.spinner.hide();
          if (res.success) {
  ...
  private changeMultiplechildCategoryStatus(data: any) {
    this.spinner.show();
    this.subDataThree = this.childCategoryService.changeMultipleChildCategoryStatus(this.selectedIds, data)
      .subscribe({
        next: (res => {
          this.spinner.hide();
          if (res.success) {
  ...
  public openConfirmDialog(type: string, data?: any) {
    if (type === 'delete') {
      ...
      dialogRef.afterClosed().subscribe(dialogResult => {
        if (dialogResult) {
          this.changeMultiplechildCategoryStatus(data);
        }
      });
