...
export class FamilyInfoComponent implements OnInit {
  ...
  private setData() {
    const data = this.storageService.getDataFromSessionStorage('MATRIMONIAL_GENERAL_FAMILY');
    if (data) {
  ...
  onFormSubmit() {
    if (this.dataForm.valid) {
      ...
      if (this.id) {
        ...
        this.storageService.storeDataToSessionStorage('MATRIMONIAL_GENERAL_FAMILY', this.dataForm.value)

        setTimeout(() => {
          this.isLoader = false;
          console.log(this.dataForm.value);
          this.router.navigate(['/add-biodata/life-partner'], {queryParamsHandling: 'merge'});
        }, 500);
      }
  ...
  private getProductById() {
    const select = ' descriptionOfFathersProfession descriptionOfMothersProfession howManyBrothersDoYouHave descriptionOfFinancialCondition howIsYourFamilysReligiousCondition writeTheNamesOfAtLeast3IslamicScholarsOfYourChoice'
